...
import api, { MOCK_CITIES, MOCK_ACTIVITIES } from '../api/client';
...
export default function ItineraryBuilder({
  ...
  const addStop = async (cityNameToAdd) => {
    const cityName = (cityNameToAdd || newStopCity).trim();
    if (!cityName) return;
    try {
      ...
      if (!stopRes.data?.stop) throw new Error('No stop returned from server');
      const dbStop = stopRes.data.stop;
      dbStop.city = cityObj;
      dbStop.activities = dbStop.activities || [];
      onUpdateTrip({ ...currentTrip, stops: [...stops, dbStop] });
      setNewStopCity('');
      setCitySuggestionsOpen(false);
      setExpandedStop(dbStop.id);
    } catch (err) {
      console.error('Failed to add stop to database, falling back to local state:', err);
      const matchedCity = MOCK_CITIES.find(c => c.name.toLowerCase() === cityName.toLowerCase()) || {
  ...
  const removeStop = async (stopId) => {
    try {
      if (!String(stopId).startsWith('stop-')) {
  ...
  const moveStop = (index, direction) => {
    ...
    onUpdateTrip({ ...currentTrip, stops: newStops.map((s, idx) => ({ ...s, order: idx + 1 })) });
  };

  const persistActivity = async (stopId, payload, localFallback) => {
    try {
      if (String(stopId).startsWith('stop-')) return localFallback;
      const res = await api.post('/activities', payload);
      return res.data.activity;
    } catch (err) {
      console.error('Failed to save activity on server, using local fallback:', err);
  ...
  const addActivity = async (stopId) => {
    ...
    const localFallback = { ...newAct, id: `act-${Date.now()}`, cost: parseFloat(newAct.cost) || 0 };
    const dbAct = await persistActivity(stopId, {
      stopId,
      name: newAct.name.trim(),
      category: newAct.category,
      cost: parseFloat(newAct.cost) || 0,
      duration: parseInt(newAct.duration) || null,
  ...
  const addPresetActivity = async (stopId, preset) => {
    const localFallback = {
      id: `act-${Date.now()}`,
      name: preset.name,
  ...
  const removeActivity = async (stopId, actId) => {
    try {
      if (!String(actId).startsWith('act-')) {
